...
module Proto(M : Simul.MCUSimul) = struct
  open DisplayTypes(M)
  ...
  let exec_8bit rs rw bus display =
    ...
    (* | (false, true) -> read_busy_flag_and_address bus bus_register display *)
    | (true, false) -> write_data_to_ram bus display
    | _ -> ()
    (* | (true, true) -> read_data_from_ram bus bus_register display *)
  ;;
