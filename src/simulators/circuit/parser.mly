...
%{
  ...
  let pin_of_string s =
    ...
    Simul.pin_of_number s
       (*failwith (sprintf "%S should be a pin" s); *)
  ;;
  ...
  let parse_component name options =
    match String.lowercase_ascii name with
      ...
      | "lcd" ->
        let opts = parse_options options [
          ("x", None); ("y", None);
          ("cs", None); ("dc", None); ("rst", None); 
          ("column_nb", None); ("line_nb", None);
        ] in
        let x = int_of_string (List.assoc "x" opts) in
        let y = int_of_string (List.assoc "y" opts) in
        let cs = Simul.pin_of_number (List.assoc "cs" opts) in
        let dc = Simul.pin_of_number (List.assoc "dc" opts) in
        let rst = Simul.pin_of_number (List.assoc "rst" opts) in
